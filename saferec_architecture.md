...
sequenceDiagram
    ...
    MS->>MS: bytesWritten >= partSize && keyFrame?
    alt Chunk complete
...
| **Chunk rotation** | On video key-frame when `bytesWritten >= partSize` |
...
- **Limitation:** Single MP4 = single rotation value. Mixed-camera sessions will have wrong orientation on some segments.
...
2. **Mixed-rotation merge.** A single MP4 can only have one `setOrientationHint`. When chunks from different cameras (with different sensor orientations) are merged, some segments will display with wrong rotation.
